...
class ChordNode(chord_pb2_grpc.chordRPCServicer):
    def __init__(self,id,port=80054,existing_ip=None,filePath=None) -> None:
        ...
        if filePath ==None: self.filePath = './data/'+str(self.node.id)+'.txt'
        else : self.filePath = './data/'+filePath
    ...
    def get_stub(self,ip):
        ...
        channel = grpc.secure_channel(ip, channel_credentials)
    ...
    def dictAddFile(self,d,p):
        data = self.file2dict(self.filePath)
        for key,value in d.items():
            data[key] = value
        self.dict2file(data)
    ...
    def get_data(self, request, context):
        if request.ip =='None':return self.dict2proto(self.file2dict(self.filePath))
            ...
            data.update({key: value for key, value in self.data.items() 
                            if  (self.hash_key(key+'2130')-request.id)%2**m < (self.node.id-request.id)%2**m})
            data.update({key: value for key, value in self.data.items() 
                if  (self.hash_key(key+'7383')-request.id)%2**m < (self.node.id-request.id)%2**m})
